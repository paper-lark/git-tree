...
class RemoteCredentialsViewModel: ObservableObject {
    ...
    init() {
        let credentials = KeychainHelper.standard.read(service: RemoteCredentialsViewModel.service,
    ...
    func persist() {
        KeychainHelper.standard.save(toModel(), service: RemoteCredentialsViewModel.service, account: RemoteCredentialsViewModel.account)
    }
}
