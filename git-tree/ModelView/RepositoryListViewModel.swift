...
class RepositoryListViewModel : ObservableObject {
    ...
    @Published var repositoryBookmarks: [String: Data] = UserDefaults.standard.dictionary(forKey: "repositories") as? [String: Data] ?? [:] {
        didSet {
    ...
    func loadBookmarks() -> Self {
        ...
        for (oldURL, bookmarkData) in currentBookmarks {
            ...
            var isStale = false
            guard let localURL = try? URL(resolvingBookmarkData: bookmarkData, bookmarkDataIsStale: &isStale), !isStale else {
                continue
            }
            print("Loaded bookmarked repository: \(localURL.absoluteString) (old URL: \(oldURL)") // FIXME: should update keys for bookmarks?
    ...
    func addRepository(fromLocalURL localURL: URL, withBookmark: Bool = true) {
        ...
        if let newRepository = RepositoryInfoModel.initWith(localPath: localURL) {
            ...
            if withBookmark {
                if let bookmarkData = try? localURL.bookmarkData(options: .minimalBookmark, includingResourceValuesForKeys: nil, relativeTo: nil) {
